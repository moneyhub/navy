...
describe('navy/index', function () {
  ...
  describe('destroy', function () {
    ...
    it('should pass navyFile from a remaining navy when reconfiguring the proxy', async function () {
      ...
      const destroyedNavyFile = { httpProxyImage: 'destroyed/proxy:1' }
      const remainingNavyFile = {
        httpProxyImage: 'remaining/proxy:1',
        httpProxyEnv: { FOO: 'bar' },
      }
      const createProvider = (path) => ({ getNavyFilePath: async () => path })
      const resolveConfigProvider = sinon.stub().callsFake(() => (navy) =>
        createProvider(navy.normalisedName === 'other' ? otherPath : envPath))
      const reconfigureHTTPProxy = sinon.stub().resolves()
      const deleteState = sinon.stub().resolves()
      const { Navy } = proxyquire.noCallThru()('../', {
        '../util/fs': {
          readdirAsync: sinon.stub().resolves(['env', 'other']),
          lstatSync: sinon.stub().returns({ isDirectory: () => true }),
        },
        './state': {
          getState: sinon.stub().resolves({ driver: 'docker-compose', configProvider: 'filesystem' }),
        ...
        '../driver': { resolveDriverFromName: sinon.stub().returns(() => driver) },
        '../config-provider': { resolveConfigProviderFromName: resolveConfigProvider },
        '../config': { getConfig: sinon.stub().returns({ externalIP: null }) },
        './plugin-interface': { loadPlugins: sinon.stub().resolves([]) },
        './middleware': { middlewareRunner: sinon.stub().resolves() },
        '../http-proxy': { reconfigureHTTPProxy },
        '../util/external-ip': { getExternalIP: sinon.stub().resolves('127.0.0.1') },
        '../util/service-host': {
          createUrlForService: sinon.stub().resolves('http://svc.local'),
          getUrlFromService: sinon.stub().returns(null),
      ...
      expect(reconfigureHTTPProxy.calledOnce).to.equal(true)
      expect(reconfigureHTTPProxy.firstCall.args[0].navies).to.eql(['other'])
      expect(reconfigureHTTPProxy.firstCall.args[0].navyFile).to.eql(remainingNavyFile)
    })

    it('should skip uninitialised remaining navies when resolving navyFile for the proxy', async function () {
      const driver = makeDriver()
      const otherPath = '/abs/other/Navyfile.js'
      const remainingNavyFile = { httpProxyImage: 'remaining/proxy:1' }
      const createProvider = (path) => ({ getNavyFilePath: async () => path })
      const resolveConfigProvider = sinon.stub().callsFake(() => (navy) =>
        createProvider(navy.normalisedName === 'other' ? otherPath : '/abs/env/Navyfile.js'))
      const reconfigureHTTPProxy = sinon.stub().resolves()
      const deleteState = sinon.stub().resolves()
      const getState = sinon.stub().callsFake((name) => {
        if (name === 'uninit') return Promise.resolve(null)
        return Promise.resolve({ driver: 'docker-compose', configProvider: 'filesystem' })
      })
      const { Navy } = proxyquire.noCallThru()('../', {
        '../util/fs': {
          readdirAsync: sinon.stub().resolves(['env', 'uninit', 'other']),
          lstatSync: sinon.stub().returns({ isDirectory: () => true }),
        },
        './state': {
          getState,
      ...
      expect(reconfigureHTTPProxy.calledOnce).to.equal(true)
      expect(reconfigureHTTPProxy.firstCall.args[0].navies).to.eql(['uninit', 'other'])
      expect(reconfigureHTTPProxy.firstCall.args[0].navyFile).to.eql(remainingNavyFile)
    })

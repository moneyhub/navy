...
describe('resolveProxyImage', function () {
  ...
  afterEach(function () {
    delete process.env.NAVY_PROXY_IMAGE
  })
  ...
  it('should use NAVY_PROXY_IMAGE env var when set', function () {
    process.env.NAVY_PROXY_IMAGE = 'envregistry/env-proxy:v2'
    expect(resolveProxyImage()).to.equal('envregistry/env-proxy:v2')
  })

  it('should give NAVY_PROXY_IMAGE env var precedence over navyFile httpProxyImage', function () {
    process.env.NAVY_PROXY_IMAGE = 'envregistry/env-proxy:v2'
    const navyFile = { httpProxyImage: 'myregistry/custom-proxy:latest' }
    expect(resolveProxyImage(navyFile)).to.equal('envregistry/env-proxy:v2')
  })

  it('should give NAVY_PROXY_IMAGE env var precedence over default', function () {
    process.env.NAVY_PROXY_IMAGE = 'override/image:tag'

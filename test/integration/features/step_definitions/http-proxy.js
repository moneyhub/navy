...
When(/I launch a service which exposes a different port which has been explicitly configured to use the HTTP proxy$/, async function () {
  await this.navy.launch(['helloworld-otherport'])
  this.serviceForProxy = 'helloworld-otherport'
})

When(/I launch a service which exposes a different port which has been explicitly configured to use the HTTP proxy, not port 80$/, async function () {
  await this.navy.launch(['helloworld-otherportnot80'])
  this.serviceForProxy = 'helloworld-otherportnot80'
})

When(/I launch a service which exposes a different port which has been explicitly configured as a HTTP proxy auto-port$/, async function () {
  await this.navy.launch(['helloworld-otherautoport'])
  this.serviceForProxy = 'helloworld-otherautoport'
})

Then(/I should be able to make a HTTP request to the service through the proxy$/, async function () {
  await retry(async () => {

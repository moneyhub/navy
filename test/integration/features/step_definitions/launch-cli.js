...
When(/I launch a service from the CLI directly$/, async function () {
  await Automator.spawn(['import']).waitForDone()
...
When(/I launch a service from the CLI prompt from the directory with my config in$/, async function () {
  const cmd = Automator.spawn(['launch'], {
...
When(/I launch a service from the CLI prompt from a directory with no config in$/, async function () {
  const cmd = Automator.spawn(['launch'], {
...
Then(/I should see that the navy was imported$/, async function () {
  expect(this.output).to.contain(`Navy "${this.navy.name}" has now been imported and initialised.`)
})

Then(/I should see that there is no compose config in the current directory$/, async function () {

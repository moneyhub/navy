...
When(/I take the service out of development$/, async function () {
...
When(/I call develop from a folder with no source code or .navyrc$/, async function () {
  this.cmd = Automator.spawn(['develop'])

  await this.cmd.waitForDone()
})

When(/I put a different service into development$/, async function () {
...
Then(/I should see that my local copy has been applied$/, async function () {
  await retry(async () => {
...
Then(/I should see that my local copy is no longer applied/, async function () {
  const url = await this.navy.url(TEST_SERVICE_NAME)
...
Then(/I should see that there are no services to put in development/, async function () {
  expect(await this.cmd.waitForDone()).to.contain('No valid .navyrc file was found in')
})

Then(/I should see that I specified an invalid development target/, async function () {

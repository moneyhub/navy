...
export default function () {
  ...
  this.When(/I get the current config$/, async function () {
    this.config = await Automator.spawn(['config', 'json']).waitForDone()
  })

  this.Then(/I should see the current config$/, async function () {
  ...
  this.When(/I get the status of all my navies$/, async function () {
  ...
  this.Then(/I should see that the default navy is correct$/, async function () {
    expect(this.status).to.contain('dev (default)')
  })

  this.When(/I change the default navy to something else$/, async function () {
  ...
  this.Then(/I should see that the default navy is something else$/, async function () {
    expect(JSON.parse(this.config)).to.eql({
  ...
  this.When(/I set the tlsCaDir/, async function () {
    await Automator.spawn(['config', 'set', 'tlsCa-dir', '/tmp']).waitForDone()
    this.tlsCaDir = await Automator.spawn(['config', 'get', 'tlsCa-dir']).waitForDone()
  })

  this.Then(/I should see that tlsCaDir is set/, async function () {
    expect(this.tlsCaDir.trim()).to.eql('/tmp')
  })

  this.Then(/I should see that the default external IP is connect$/, async function () {
  ...
  this.When(/I override the external IP using an environment variable$/, async function () {
  ...
  this.Then(/I should see that the external IP is what I set the environment variable to$/, async function () {
    expect(this.ip.trim()).to.equal('192.168.2.102')
  })

  this.When(/I change the external IP to something else$/, async function () {
  ...
  this.Then(/I should see that all services were reconfigured with the new external IP/, async function () {
    expect(JSON.parse(this.config)).to.eql({
      defaultNavy: 'dev',
